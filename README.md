...
- 📈 **實時行情監控** - 整合 Binance WebSocket，秒級同步主流幣種最新價格。
  - 支援閃爍動效（綠色漲、紅色跌），掌握瞬息萬變的行情。
- 🤖 **AI 智能分析** - 整合 Google Gemini 1.5 Pro/Flash 引擎。
  - **按需分析**：一鍵獲取當前交易對的市場趨勢、支撐壓力位與風險等級。
  - **智能建議**：結合 AI 解析，輔助交易者進行客觀判斷。
...
│   │   ├── Settings.tsx     # 設定頁面
│   │   ├── PriceTicker.tsx  # 實時行情報價元件
│   │   ├── AIAnalysis.tsx   # AI 行情分析報告元件
│   │   └── Button.tsx       # 可複用按鈕元件
│   ├── services/            # 業務邏輯服務
│   │   ├── cryptoService.ts # WebSocket 行情連線服務
│   │   └── aiService.ts     # Google AI API 整合服務
...
TradeGuard 深度整合了 **Google Gemini** AI 引擎，為交易者提供數據驅動的洞察：

- **引擎版本**：Gemini 1.5 Flash (預設)，兼顧速度與準確度。
- **獲取報告**：在「建立交易」後的檢查清單頁面，點擊「獲取報告」按鈕。
- **分析內容**：
  - **市場總結**：當前 K 線形態與情緒解析。
  - **技術指標**：判定目前為「看多」、「看空」或「中性」。
  - **關鍵價位**：自動識別當前的壓力位與支撐位。
  - **風險評級**：評估當前進場的潛在風險等級。

> 💡 **提示**：AI 分析僅供參考，不代表投資建議。建議結合多個 AI 模型與個人判斷。
