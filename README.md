...
- 🤖 **AI 驅動分析** - 整合 Google Gemini AI 提供智慧建議
...
| **AI 服務** | Google Gemini API | ^1.29.1 |
...
- Node.js >= 18.x
- npm 或 yarn
- Google Gemini API Key ([取得方式](https://ai.google.dev/))
...
TradeGuard 整合了 Google Gemini 2.5 Flash 模型，提供以下功能：

- **智慧生成檢查清單** - 根據您的交易策略描述，自動生成 5-8 個具體的檢查項目
- **結構化輸出** - 使用 JSON Schema 確保生成的檢查項目格式正確

#### 使用範例

```typescript
import { generateChecklistFromStrategy } from './services/geminiService';

const checklist = await generateChecklistFromStrategy(
  "我想交易 BTC/USDT，基於 RSI 超賣訊號做多"
);
// 返回: [{ id: '1', text: '確認 RSI 是否低於 30' }, ...]
```
...
訪問 [Google AI Studio](https://ai.google.dev/) 並註冊帳號，即可免費取得 API Key。
