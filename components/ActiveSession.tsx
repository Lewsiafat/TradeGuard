            ...
            >
               ...
               <div className="flex justify-between items-start mb-2">
                 ...
                 <span className={`px-2 py-1 rounded text-xs font-bold ${
                    trade.status === TradeStatus.CHECKING ? 'bg-yellow-900/30 text-yellow-400' : 'bg-indigo-900/30 text-indigo-400'
                 }`}>
        ...
        <div className="bg-crypto-card border border-gray-700 rounded-2xl p-6 shadow-2xl relative overflow-hidden">
          ...
          <div className="space-y-4">
            {checklistTemplate.map((item) => (
              <label 
                key={item.id} 
                className={`flex items-start p-4 rounded-xl border transition-all cursor-pointer select-none ${
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div>
             ...
             <input
                ...
                placeholder="0.00"
                className={`w-full bg-gray-900 border-2 rounded-lg px-4 py-2 font-bold focus:outline-none ${
                    parseFloat(closeData.pnl) > 0 ? 'border-emerald-500 text-emerald-400' : parseFloat(closeData.pnl) < 0 ? 'border-rose-500 text-rose-400' : 'border-gray-700 text-white'
                }`}
              />
          </div>
          <div>
             ...
             <input
                ...
                placeholder="0.00"
                className={`w-full bg-gray-900 border-2 rounded-lg px-4 py-2 font-bold focus:outline-none ${
                    parseFloat(closeData.pnlPercentage) > 0 ? 'border-emerald-500 text-emerald-400' : parseFloat(closeData.pnlPercentage) < 0 ? 'border-rose-500 text-rose-400' : 'border-gray-700 text-white'
                }`}
              />
          </div>
        </div>
